...
document.getElementById("closeMainMenu").addEventListener("click", () => { document.getElementById("mainMenuModal").style.display = "none"; });
    ...
    adController.show().then((result) => { 
        if (result.done) { 
            fetch('https://ninjabridgeapi.duckdns.org/api/score/promocode', {
                method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId, code: codeInput })
                ...
                if(data.success) {
                    ...
                    loadPlayerData();
                } else { if(tg && tg.showAlert) tg.showAlert(`❌ ${data.message}`); }
...
function isMenuOpen() { 
    return (document.getElementById("shopModal") && document.getElementById("shopModal").style.display === "block") || 
           ...
           (document.getElementById("vipInfoModal") && document.getElementById("vipInfoModal").style.display === "flex") ||
           (document.getElementById("dailyRewardModal") && document.getElementById("dailyRewardModal").style.display === "flex") ||
           (document.getElementById("csgoCaseModal") && document.getElementById("csgoCaseModal").style.display === "flex"); 
}
...
inviteBtn.style.position = "absolute"; inviteBtn.style.top = "215px"; inviteBtn.style.left = "20px"; inviteBtn.style.padding = "8px 12px"; inviteBtn.style.fontSize = "13px"; inviteBtn.style.fontWeight = "bold"; inviteBtn.style.backgroundColor = "#e74c3c"; inviteBtn.style.color = "white"; inviteBtn.style.border = "none"; inviteBtn.style.borderRadius = "5px"; inviteBtn.style.cursor = "pointer"; inviteBtn.style.boxShadow = "0 2px 4px rgba(0,0,0,0.2)"; inviteBtn.style.zIndex = "100";
...
let yeeyImg = new Image(); yeeyImg.src = 'yeey.png'; 
const yeeySound = new Audio('yeey.mp3'); yeeySound.volume = 1.0;
...
document.getElementById("watchEarnBtn").addEventListener("click", () => {
    ...
    adController.show().then((result) => { 
        if (result.done) { 
            ...
            fetch('https://ninjabridgeapi.duckdns.org/api/score/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId, firstName: tgUserName, score: score, groupId: tgGroupId, earnedCoins: earned, earnedGems: 0 }) }).then(() => { sessionEarnedCoins = 0; }).catch(e => {});
...
let easterMessage = ""; let easterMessageTimer = 0; let nextMessageMilestone = 500;
...
// 🔥 KAPANMA KORUMASI (Rage Quit yapanların skoru zorla gönderilir)
window.addEventListener("beforeunload", () => {
    if (score > 0) {
        let data = new Blob([JSON.stringify({ userId: tgUserId, firstName: tgUserName, score: score, groupId: tgGroupId, earnedCoins: sessionEarnedCoins, earnedGems: 0 })], { type: 'application/json' });
        navigator.sendBeacon('https://ninjabridgeapi.duckdns.org/api/score/save', data);
    }
});
...
function checkDuelStatus() {
    fetch(`https://ninjabridgeapi.duckdns.org/api/score/duel-status/${tgUserId}?t=${Date.now()}`).then(res => res.json()).then(data => {
        if (data && data.active) {
...
// KAYIT İŞLEMİ (Elde edilen jetonu sıfırlar ki çift kayıt olmasın)
function saveScoreToAPI() { 
    fetch('https://ninjabridgeapi.duckdns.org/api/score/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId, firstName: tgUserName, score: score, groupId: tgGroupId, earnedCoins: sessionEarnedCoins, earnedGems: 0 }) })
    .then(() => { sessionEarnedCoins = 0; })
    .catch(e => {}); 
}
...
function generatePlatform() {
    ...
    if (score >= 4000) {
        ...
        lastPlatform.fireGapX = gapStartX + gapWidth / 2; 
        lastPlatform.fireY = -150;
        lastPlatform.fireTimer = Math.random() * 60; 
...
function animate(timestamp) {
  ...
  switch (phase) {
    ...
    case "walking": {
      ...
      let isImmuneFire = (currentSkin === "cehennem_zebanisi" || currentSkin === "efsanevi"); 

      monsters.forEach(m => { 
          if (m.dead) return; 
          if (m.world === 2) { 
              if (Math.abs(heroX - m.x) < 15 && Math.abs(m.y) < 35) { if(!isImmuneWorld2) { phase = "dead_monster"; saveScoreToAPI(); fallSound.currentTime = 0; fallSound.play().catch(e=>{}); } } 
          } else if (m.world === 3) { 
              let p = platforms[m.platformIndex]; if (p && Math.abs(heroX - m.x) < 15) { if(!isImmuneWorld3) { phase = "dead_monster"; saveScoreToAPI(); fallSound.currentTime = 0; fallSound.play().catch(e=>{}); } } 
          } else if (m.world === 4) {
              if (Math.abs(heroX - m.x) < 15 && m.y > -50 && m.y < 10) {
                  if(!isImmuneWorld4) { phase = "dead_monster"; saveScoreToAPI(); fallSound.currentTime = 0; fallSound.play().catch(e=>{}); }
              }
          }
      });
      ...
      platforms.forEach(p => {
          if(p.hasFireRain && !isImmuneFire) {
              if (Math.abs(heroX - p.fireGapX) < 15) { 
                  let heroActualY = canvasHeight - platformHeight - (heroHeight / 2);
                  if (Math.abs(p.fireY - heroActualY) < 30) { 
                      phase = "dead_monster"; 
                      saveScoreToAPI(); // 🔥 ANINDA KAYIT
...
function draw() { 
    ...
    if (yeeyAnimTimer > 0) {
        ...
        let yPos = canvasHeight - platformHeight + yOffset;
        if (yeeyImg.complete && yeeyImg.naturalWidth !== 0) {
            ctx.drawImage(yeeyImg, yeeyX - 40, yPos, 80, 80);
        } else {
            ctx.font = "50px Arial"; ctx.textAlign = "center"; ctx.fillText("👽", yeeyX, yPos + 60);
        }
        ctx.restore();
        yeeyAnimTimer--;
    }
...
function drawPlatforms() { 
  ...
  platforms.forEach((p) => { 
      ...
      if (p.hasChest) {
          ...
          if (!p.chestOpened) { ctx.fillText("🎁", 0, 0); } else { ctx.globalAlpha = 0.5; ctx.fillText("✨", 0, -10); } ctx.restore();
      }
...
// 🔥 PETİN KONUMU AYARLANDI (Ninjaya yapışmasın diye daha sola ve yukarıya alındı)
    ...
    if (window.confirm("30 Elmas ile Premium Kasa açıyorsun. Çark dönecek! Onaylıyor musun?")) {
        ...
        .then(data => { 
            if (data.success) { 
                playerGems -= 30; 
                updateCoinUI();
                playCsgoAnimation(data.rewardGems, data.message); 
            } else { 
                isBoxOpening = false;
...
function playCsgoAnimation(wonGem, messageText) {
    ...
    for(let i=0; i<totalItems; i++) {
        ...
        if (gemAmount === 100) { bgColors = "linear-gradient(135deg, #ff00cc, #333399)"; textShadow = "0 0 10px white"; } 
        else if (gemAmount === 50) { bgColors = "linear-gradient(135deg, #f1c40f, #e67e22)"; } 
        else if (gemAmount === 40) { bgColors = "linear-gradient(135deg, #9b59b6, #8e44ad)"; } 
        else if (gemAmount === 25) { bgColors = "linear-gradient(135deg, #3498db, #2980b9)"; } 
        else { bgColors = "linear-gradient(135deg, #7f8c8d, #34495e)"; } 
...
window.buyVIP = function() {
    const btn = document.getElementById("buyVipBtn"); btn.innerText = "⏳ Fatura İsteniyor..."; btn.disabled = true;
    fetch('https://ninjabridgeapi.duckdns.org/api/score/create-vip-invoice', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId }) })
    .then(res => res.json()).then(data => {
...
window.checkDailyReward = function() {
    ...
    .then(res => res.json())
    .then(data => { if(data.success && !data.claimedToday) { renderDailyCalendar(data.currentDay, data.isPremium); document.getElementById('dailyRewardModal').style.display = 'flex'; } }).catch(e => console.log("Takvim Hatası:", e));
};
...
window.renderDailyCalendar = function(currentDay, isPremium) {
    ...
    for(let i = 1; i <= 30; i++) {
        let isPast = i < currentDay; let isToday = i === currentDay; let bgColor = isToday ? "#f39c12" : (isPast ? "#27ae60" : "#34495e"); let icon = "❌";
        if (isPremium) { icon = (i % 5 === 0) ? "💎" : "🪙"; if(i === 30) icon = "👑"; } else { if (normalDays.includes(i)) icon = (i === 15 || i === 30) ? "💎" : "🪙"; }
        if (isPast) icon = "✅";
...
window.claimDailyReward = function() {
    const btn = document.getElementById("claimDailyBtn"); btn.innerText = "⏳ Bekle..."; btn.disabled = true;
    fetch('https://ninjabridgeapi.duckdns.org/api/score/claim-daily', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId }) })
    .then(res => res.json()).then(data => {
        if(data.success) {
            if(data.rewardCoins > 0 || data.rewardGems > 0) { playerCoins += data.rewardCoins; playerGems += data.rewardGems; updateCoinUI(); }
            if(tg && tg.showAlert) tg.showAlert(data.message); document.getElementById('dailyRewardModal').style.display = 'none';
        }
    }).catch(e => { btn.innerText = "🎁 ÖDÜLÜ AL"; btn.disabled = false; if(tg && tg.showAlert) tg.showAlert("Hata: " + e.message); });
};
