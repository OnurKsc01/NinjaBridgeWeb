...
// 🔥 DÜELLO BİLDİRİM MANTIĞI BURADA ÇALIŞIYOR
    ...
    fetch(`https://ninja-bridge-api.onrender.com/api/score/duel-status/${tgUserId}`)
        ...
        .then(data => {
            if (data && data.active) {
                ...
                // Rakip eğer oynamış ve yanmışsa (-1 değilse) ölüm afişini göster
                if (opponentTargetScore !== -1) {
                    ...
                    if (banner) {
                        ...
                        // 4 saniye sonra ekrandan yavaşça kaybolsun ki oyun alanını engellemesin
                        setTimeout(() => {
  ...
  switch (phase) {
    ...
    case "falling": {
      ...
      if (heroY > maxHeroY) {
        ...
        // 🔥 DÜELLO REKLAM BYPASS: Düellodaysa reklama girmeden direk bitir
        if (isDuelMode) {
            phase = "dead_options"; 
...
window.equipPet = function(petKey) { if (phase !== "waiting") { if(tg && tg.showAlert) tg.showAlert("Oyun esnasında yoldaş değiştirilemez!"); return; } fetch('https://ninja-bridge-api.onrender.com/api/score/equippet', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId, skinName: petKey }) }).then(res => res.json()).then(data => { if(data.success) { currentPet = petKey; renderShop(); draw(); } }); }
