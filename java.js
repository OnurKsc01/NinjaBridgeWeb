...
const skinData = {
  ...
  "hayalet": { name: "Hayalet", price: 10, body: "#ffffff", bandana: "#cccccc", alpha: 0.4 },
...
const bgData = {
  ...
  "volkan": { name: "Volkan Dağı", price: 250, top: "#2b0000", bottom: "#1a0000", hill1: "#ff3300", hill2: "#cc0000", tree: "#1a0000", leaves: ["#ff6600", "#ff3300", "#cc0000"], isDark: true }
};
...
let platforms = [], sticks = [], trees = []; let score = 0, combo = 0; let currentMonkeyLives = 0; 

const canvasWidth = 375, canvasHeight = 375, platformHeight = 100; const heroDistanceFromEdge = 10, paddingX = 100;
const backgroundSpeedMultiplier = 0.2; const hill1BaseHeight = 100, hill1Amplitude = 10, hill1Stretch = 1;
...
function getPerfectAreaSize() {
    if (currentPet === "kedi") { let lvl = ownedPets["kedi"] || 1; return 20 + (lvl * 5); }
    return 10;
}
...
function generateTree() {
  let bg = bgData[currentBg] || bgData["default"]; const minimumGap = 30, maximumGap = 150;
  let furthestX = trees.length > 0 ? trees.last().x : 0;
    ...
    case "turning":
      ...
      if (sticks.last().rotation > 90) {
        ...
        if (nextPlatform) {
          ...
          scoreElement.innerText = score;
          if (perfectHit) { perfectElement.style.opacity = 1; setTimeout(() => (perfectElement.style.opacity = 0), 1200); }
    ...
    case "falling":
      ...
      if (heroY > platformHeight + 100 + (window.innerHeight - canvasHeight) / 2) {
        if (currentMonkeyLives > 0) {
            ...
            if (mStats.bonusCoins > 0) { playerCoins += mStats.bonusCoins; updateCoinUI(); }
            ...
            draw(); setTimeout(() => { perfectElement.style.opacity = 0; phase = "waiting"; }, 1500);
            return;
        }
...
function thePlatformTheStickHits() {
  ...
  if (platformTheStickHits && platformTheStickHits.x + platformTheStickHits.w / 2 - pArea / 2 < stickFarX && stickFarX < platformTheStickHits.x + platformTheStickHits.w / 2 + pArea / 2) return [platformTheStickHits, true];
  return [platformTheStickHits, false];
}

function draw() {
  ctx.save(); ctx.clearRect(0, 0, window.innerWidth, window.innerHeight); drawBackground();
...
function drawSticks() {
  ...
  sticks.forEach((stick) => {
    ...
    ctx.beginPath(); ctx.lineWidth = 3; ctx.strokeStyle = stickColor; if(glowEffect > 0) { ctx.shadowBlur = glowEffect; ctx.shadowColor = stickColor; }
...
function drawPlatforms() {
  ...
  platforms.forEach(({ x, w }) => {
    ctx.save(); ctx.fillStyle = platformColor; if(bg.isDark) { ctx.shadowBlur = 15; ctx.shadowColor = glowColor; ctx.strokeStyle = glowColor; ctx.lineWidth = 2; }
    ctx.fillRect(x, canvasHeight - platformHeight, w, platformHeight + (window.innerHeight - canvasHeight) / 2);
    if(bg.isDark) { ctx.strokeRect(x, canvasHeight - platformHeight, w, platformHeight + (window.innerHeight - canvasHeight) / 2); } ctx.restore();
    if (sticks.last().x < x) { ctx.fillStyle = "red"; let pArea = getPerfectAreaSize(); ctx.fillRect(x + w / 2 - pArea / 2, canvasHeight - platformHeight, pArea, pArea); }
  });
}

function drawHero() {
  let skin = skinData[currentSkin] || skinData["default"];
  ctx.save(); ctx.globalAlpha = skin.alpha; ctx.fillStyle = skin.body; ctx.translate(heroX - heroWidth / 2, heroY + canvasHeight - platformHeight - heroHeight / 2);
  drawRoundedRect(-heroWidth / 2, -heroHeight / 2, heroWidth, heroHeight - 4, 5); ctx.fillStyle = skin.body; if(skin.body === "#ffffff") ctx.fillStyle = "#cccccc"; 
  const legDistance = 5; ctx.beginPath(); ctx.arc(legDistance, 11.5, 3, 0, Math.PI * 2, false); ctx.fill(); ctx.beginPath(); ctx.arc(-legDistance, 11.5, 3, 0, Math.PI * 2, false); ctx.fill();
  ctx.beginPath(); ctx.fillStyle = "white"; if(skin.body === "#ffffff") ctx.fillStyle = "black"; ctx.arc(5, -7, 3, 0, Math.PI * 2, false); ctx.fill();
  ctx.fillStyle = skin.bandana; ctx.fillRect(-heroWidth / 2 - 1, -12, heroWidth + 2, 4.5);
  ctx.beginPath(); ctx.moveTo(-9, -14.5); ctx.lineTo(-17, -18.5); ctx.lineTo(-14, -8.5); ctx.fill(); ctx.beginPath(); ctx.moveTo(-10, -10.5); ctx.lineTo(-15, -3.5); ctx.lineTo(-5, -7); ctx.fill();
  ctx.restore();
}

function drawPet() {
  if (currentPet === "default") return; let pet = petData[currentPet]; ctx.save();
  let bounce = (phase === "walking" || phase === "transitioning") ? Math.abs(Math.sin(Date.now() / 100)) * 6 : 0;
  ctx.translate(heroX - 28, heroY + canvasHeight - platformHeight - 5 - bounce); ctx.font = "20px Arial"; ctx.fillText(pet.emoji, -10, 5); ctx.restore();
}
...
function drawBackground() {
  let bg = bgData[currentBg] || bgData["default"]; var gradient = ctx.createLinearGradient(0, 0, 0, window.innerHeight); gradient.addColorStop(0, bg.top); gradient.addColorStop(1, bg.bottom);
  ctx.fillStyle = gradient; ctx.fillRect(0, 0, window.innerWidth, window.innerHeight); drawHill(hill1BaseHeight, hill1Amplitude, hill1Stretch, bg.hill1); drawHill(hill2BaseHeight, hill2Amplitude, hill2Stretch, bg.hill2); trees.forEach((tree) => drawTree(tree.x, tree.color, bg.tree));
}
...
// ------------------------------------
// MARKET SİSTEMİ (KÜÇÜLTÜLMÜŞ TASARIM)
// ------------------------------------
...
window.convertGems = function() {
    ...
    tg.showConfirm("1000 Jetonunu verip 1 Elmas 💎 alacaksın. Onaylıyor musun?", function(agreed) {
        if(agreed) { fetch('https://ninja-bridge-api.onrender.com/api/score/convert', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: tgUserId }) }).then(res => res.json()).then(data => { if(data.success) { playerCoins -= 1000; playerGems += 1; updateCoinUI(); renderShop(); tg.showAlert("💎 Elmas Eklendi!"); } }); }
    });
}
...
document.getElementById("leaderboardBtn").addEventListener("click", (e) => {
    e.stopPropagation(); document.getElementById("leaderboardModal").style.display = "block"; const list = document.getElementById("scoreList"); list.innerHTML = "<li style='text-align:center;'>Yükleniyor... ⏳</li>";
